...
- **`club_team_players.number` is a write-once snapshot, not a live value** — it's copied from `player_profiles.number` only when a player is added to a team roster; nothing ever re-syncs it afterwards, so editing a player's number later left `team_list`'s "Leden tonen" panel, `team_positions`, and `/api/teams/<id>/players` showing stale numbers until an admin re-saved the team's edit form (which does a full roster DELETE+re-INSERT). Fixed by having those read paths `COALESCE(pp.number, ctp.number)` (join back to `player_profiles`) instead of writing a sync step — always prefer joining to the canonical table over trusting a per-row snapshot column.
