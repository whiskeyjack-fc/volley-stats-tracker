...
- **`flushQueue` must treat `!r.ok` HTTP responses the same as thrown network exceptions** — both should increment the failure streak and trigger a user-visible toast after the threshold is reached.
