...
- **Sortable table columns** (`data-sort-type="text|number|date"` on `<th>`, click-to-sort via inline JS) persist their active sort client-side in `localStorage` under a page-specific key (e.g. `roster-sort-v1`, `kit-sort-v1`), restored on `DOMContentLoaded`. This is separate from the filter cookie mechanism since sorting never touches the server. The `Alles tonen` reset link must clear this key via an inline `onclick` (`localStorage.removeItem(...)`) alongside its normal cookie-clearing redirect.
